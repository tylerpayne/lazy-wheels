...
pypi-publish:
  needs: [finalize]
  runs-on: ubuntu-latest
  if: ${{ always() && !failed() && !cancelled() && !contains(fromJSON(inputs.plan).skip, 'pypi-publish') }}
  environment: pypi
  steps:
    # ...
```
...
- **Use `always() && !failed() && !cancelled()`** in the `if` condition for jobs that follow skippable jobs. Without this, a skipped upstream job causes the downstream job to be skipped too.
